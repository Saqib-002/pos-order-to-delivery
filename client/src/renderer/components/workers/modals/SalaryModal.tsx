...
export const SalaryModal = ({ isOpen, onClose, worker, onAddRecord, onUpdateRecord, onDeleteRecord, fetchRecords }: Props) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50 shrink-0">
          <div>
            <h3 className="text-lg font-bold text-gray-800">Salary Management</h3>
        ...
        <div className="p-4 border-b border-gray-100 flex gap-4 bg-white shrink-0">
            <button onClick={() => handleTabChange('list')} className={`px-4 py-2 rounded-lg text-sm font-medium ${activeTab === 'list' ? 'bg-black text-white' : 'bg-gray-100 text-gray-600'}`}>History</button>
            <button onClick={() => handleTabChange('form')} className={`px-4 py-2 rounded-lg text-sm font-medium ${activeTab === 'form' ? 'bg-black text-white' : 'bg-gray-100 text-gray-600'}`}>
        ...
        <div className="p-6 overflow-y-auto">
            {activeTab === 'list' ? (
                            ...
                            {records.map(r => (
                                <tr key={r.id}>
                                    ...
                                    <td className="p-2 text-right flex justify-end gap-2">
                                        <button onClick={() => handleEditClick(r)} className="text-blue-500 hover:bg-blue-50 p-1 rounded"><EditIcon className="size-4"/></button>
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
                    ...
                    <CustomInput name="irpf" type="number" step="0.01" label="IRPF" value={formData.irpf} onChange={e => setFormData({...formData, irpf: parseFloat(e.target.value)})} />
                    ...
                    <div className="col-span-2 mt-4 flex justify-end gap-2">
                         <CustomButton type="button" variant="secondary" onClick={() => handleTabChange('list')} label="Cancel" />
                         <CustomButton type="submit" label={editingId ? "Update Salary Record" : "Save Salary Record"} />
                    </div>
                </form>
            )}
        </div>
