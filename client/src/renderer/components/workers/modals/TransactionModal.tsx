...
export const TransactionModal = ({
  ...
  const totalCustomerGiven = paymentMethods.reduce(
    (sum, method) =>
      sum + (Number(method.amountTendered) || Number(method.amount) || 0),
    0
  );
  ...
  const handleAddPayment = () => {
    ...
    const existingNonExistingMethodIndex = paymentMethods.findIndex(
      (method) => method.type === selectedPaymentType && !method.isExisting
    );

    if (existingNonExistingMethodIndex !== -1) {
      const updatedMethods = [...paymentMethods];
      const currentAmount =
        Number(updatedMethods[existingNonExistingMethodIndex].amount) || 0;
      updatedMethods[existingNonExistingMethodIndex].amount =
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-600">
                      ...
                      {method.isExisting && method.date && (
                        <span className="ml-2 text-xs text-gray-500">
                          (
                          {new Date(method.date).toLocaleDateString("es-ES", {
                            day: "2-digit",
                            month: "2-digit",
                            year: "numeric",
                          })}{" "}
                          {new Date(method.date).toLocaleTimeString("es-ES", {
                            hour: "2-digit",
                            minute: "2-digit",
                          })}
                          )
