...
const CreateVariantModal: React.FC<CreateVariantModalProps> = ({
  ...
  const handleVariantImageChange = async (
    ...
    setCompressingMap((prev) => ({ ...prev, [variantId]: true }));
    setCompressInfoMap((prev) => ({ ...prev, [variantId]: null }));
    ...
    setCompressInfoMap((prev) => ({ ...prev, [variantId]: info }));
    setCompressingMap((prev) => ({ ...prev, [variantId]: false }));
  };
                    ...
                    <div className="w-32 flex-shrink-0">
                      ...
                      <div className="relative border-2 border-dashed border-gray-300 rounded-lg p-1 hover:border-blue-400 transition-colors cursor-pointer bg-gray-50 hover:bg-gray-100  flex items-center justify-center touch-manipulation">
                        <input
                          ...
                          disabled={!!compressingMap[variant.id]}
                        />
                        {compressingMap[variant.id] ? (
                      ...
                      {compressInfoMap[variant.id] && (
                        <p className="text-xs text-amber-600 font-medium mt-1 flex flex-wrap gap-1">
                          <span>{(compressInfoMap[variant.id]!.original / 1024).toFixed(0)} KB → {(compressInfoMap[variant.id]!.compressed / 1024).toFixed(0)} KB</span>
                          <span className="text-gray-400 font-normal">
                            ({t("common.compressedBy").replace("{percent}", String(Math.round((1 - compressInfoMap[variant.id]!.compressed / compressInfoMap[variant.id]!.original) * 100)))})
                          </span>
                          {compressInfoMap[variant.id]!.width && compressInfoMap[variant.id]!.height && (
          ...
          {editingVariant && (
            <div className="mb-6">
              <button
                type="button"
                onClick={async () => {
                  await fetchAssociatedProductsByVariantId(
                    token,
                    editingVariant.id,
