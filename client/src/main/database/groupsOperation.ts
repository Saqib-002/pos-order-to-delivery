  ...
  static async createGroup(
  ...
  ) {
    {
      ...
      try {
        ...
        await trx("group_items").insert(newGroupItems);
        await trx.commit();
        return {
          group: newGroup,
    ...
    try {
      ...
      rows.forEach((row) => {
        const group = {
          ...
          name: row.name,
          color: row.color,
          forProduct: Boolean(row.forProduct),
          isRemovalGroup: Boolean(row.isRemovalGroup),
  ...
  static async getAttachProductsByGroupId(groupId: string) {
    try {
      const products = await db("products_groups")
        .join("products", "products_groups.productId", "=", "products.id")
        .join(
          "sub_categories",
