...
import { Worker, WorkerFilters, WorkerSalary, SalaryFilters, PaginatedResult } from "@/types/workers";
import { randomUUID } from "crypto";

export class WorkerDatabaseOperations {
    // --- Workers ---

    static async createWorker(data: Omit<Worker, "id" | "createdAt" | "updatedAt">): Promise<Worker> {
        try {
    ...
    static async getWorkers(filters: WorkerFilters): Promise<PaginatedResult<Worker>> {
        try {
            const { page = 1, pageSize = 10, search, paymentMethod } = filters;
            ...
            const workers = await query.orderBy("createdAt", "desc").limit(pageSize).offset(offset);
    ...
    static async updateWorker(id: string, updates: Partial<Worker>): Promise<Worker> {
        try {
            const now = new Date().toISOString();
            const { id: _id, createdAt, ...validUpdates } = updates as any;
            await db("workers").where("id", id).update({ ...validUpdates, updatedAt: now });
    ...
    static async addSalaryRecord(data: Omit<WorkerSalary, "id">): Promise<WorkerSalary> {
        try {
            const id = randomUUID();
