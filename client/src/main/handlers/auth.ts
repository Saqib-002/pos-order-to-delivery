...
): Promise<{ status: boolean; data?: Omit<User, "syncAt">; error?: string }> {
    try {
...
export async function loginUser(
    ...
    token: string;
    status: boolean;
    user: Omit<User, "password" | "syncAt">;
}> {
    const JWT_SECRET = process.env.JWT_SECRET || "your-secret-key";
    try {
        const { token, user } = await UserDatabaseOperations.loginUser(
            username,
            userPassword
        );
        ...
        return {
            token: "",
            user: {} as Omit<User, "password" | "syncAt">,
            status: false,
        };
    }
}
...
export async function getUsers(
    ...
    status: boolean;
    error?: string;
    data: Omit<User, "password" | "syncAt">[];
}> {
    if (!isAdmin(token)) {
        return {
            status: false,
...
): Promise<{ status: boolean; error?: string; data: Omit<User, "syncAt"> }> {
    if (!isAdmin(token)) {
        return {
            status: false,
            error: "Unauthorized: Admin access required",
