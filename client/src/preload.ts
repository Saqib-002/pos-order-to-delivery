...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  getIsSyncMaster: () => ipcRenderer.invoke("get-is-sync-master"),
  setIsSyncMaster: (enabled: boolean) =>
    ipcRenderer.invoke("set-is-sync-master", enabled),
  // CDN & Driver URL
  ...
  addMultipleVehicleMaintenance: (token: string, records: any[]) =>
    ipcRenderer.invoke("add-multiple-vehicle-maintenance", token, records),
  updateMultipleVehicleMaintenancePayments: (
    ...
    ipcRenderer.invoke(
      "update-multiple-vehicle-maintenance-payments",
      token,
      maintenanceIds,
      paymentType
    ),
  ...
  deleteOrder: (token: string, id: string, cancelNote?: string) =>
    ipcRenderer.invoke("delete-order", token, id, cancelNote),
  ...
  getPendingOrdersByDeliveryPerson: (token: string, personId: string) =>
    ipcRenderer.invoke(
  ...
  updateDeliveryPerson: (token: string, id: string, deliveryPersonData: any) =>
  ...
  ) => ipcRenderer.invoke("update-menu-priorities", token, menus),
  ...
  deleteIncomeSource: (token: string, incomeSourceId: string) =>
    ipcRenderer.invoke("delete-income-source", token, incomeSourceId),
  getAllIncomeSources: (token: string) =>
    ipcRenderer.invoke("get-all-income-sources", token),
  ...
  deleteInventoryProduct: (token: string, productId: string) =>
    ipcRenderer.invoke("delete-inventory-product", token, productId),
  getAllInventoryProducts: (token: string) =>
