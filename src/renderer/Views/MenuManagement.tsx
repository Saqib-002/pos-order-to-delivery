...
export const MenuManagement: React.FC<MenuManagementProps> = ({ token }) => {
  ...
  const handleAddItem = async (e: React.FormEvent) => {
    ...
    try {
      const item = await (window as any).electronAPI.createMenuItem(token, newItem);
          ...
          {filteredItems.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="mt-1 text-sm text-gray-500">
                {menuItems.length === 0 ? 'Get started by adding your first menu item.' : 'Try adjusting your search or category filter.'}
              </p>
            </div>
          ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setEditingItem({ ...editingItem, isAvailable: e.target.value === 'available' })}
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-indigo-600 focus:border-indigo-600"
                  >
                    <option value="available">Available</option>
                    <option value="unavailable">Unavailable</option>
