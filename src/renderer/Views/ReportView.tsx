...
export const ReportView: React.FC<ReportViewProps> = ({
    ...
    useEffect(() => {
        const fetchAnalytics = async () => {
            const res = await (window as any).electronAPI.getOrderAnalytics(
                token,
                {
                    dateRange,
                    selectedDate,
            ...
            const analyticsRes = {
                ...
                successRate:
                    totalOrders > 0
                        ? Math.round(
                              (res.data.totalDelivered / totalOrders) * 100
                          )
                        : 0,
            };
    ...
    const statusDistribution = [
        {
            status: "Delivered",
            count: analytics?.totalDelivered || 0,
            color: "bg-green-500",
        },
        {
            status: "In Kitchen",
            count: analytics?.totalSentToKitchen || 0,
            color: "bg-orange-500",
        },
        {
            status: "Ready for Delivery",
            count: analytics?.totalReadyForDelivery || 0,
            color: "bg-yellow-500",
        },
        {
            status: "Out for Delivery",
            count: analytics?.totalOutForDelivery||0,
            color: "bg-blue-500",
        },
        {
            status: "Cancelled",
            count: analytics?.totalCancelled|| 0,
            color: "bg-red-500",
        },
                    ...
                    <div className="flex justify-between items-center">
                        <div>
                            ...
                            <p className="text-gray-600 mt-1">
                                Comprehensive insights into order performance
                                and trends
                            </p>
                        </div>
                                ...
                                <div className="flex items-center space-x-2">
                                    <div className="w-32 bg-gray-200 rounded-full h-2">
                                        <div
                                            ...
                                            style={{
                                                width: `${analytics.topItems.some((t) => t.count > 0) ? (item.count / Math.max(...analytics.topItems.map((t) => t.count))) * 100 : 0}%`,
                                            }}
                                        ></div>
                                    </div>
                    ...
                    {analytics?.orders.length === 0 ? (
                        <div className="text-center py-12">
                            ...
                            <p className="mt-1 text-sm text-gray-500">
                                Try adjusting your date range or period
                                selection.
                            </p>
                        </div>
                    ) : (
                        <div className="overflow-x-auto">
                            ...
                            {analytics?.orders && analytics?.orders.length > 20 && (
                                <div className="px-6 py-4 border-t border-gray-200 text-center text-sm text-gray-500">
                                    Showing first 20 orders of{" "}
                                    {analytics.orders.length} total orders
                                </div>
                            )}
                        </div>
                    )}
