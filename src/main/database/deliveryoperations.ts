...
export class DeliveryDatabaseOperations {
  ...
  static async updateDeliveryPerson(
    ...
    try {
      ...
      const updatedPerson = await localDb("delivery_persons").where("id", id).first();
  ...
  static async assignDeliveryPerson(orderId: string, deliveryPersonId: string): Promise<void> {
  ...
  static async getOrdersByDeliveryPerson(deliveryPersonId: string): Promise<any[]> {
    try {
  ...
  static async getDeliveryPersonStats(deliveryPersonId: string): Promise<any> {
    try {
      const stats = await localDb("orders")
        .where("deliveryPersonId", deliveryPersonId)
        .andWhere("isDeleted", false)
        .select(
          localDb.raw("COUNT(*) as totalAssigned"),
          localDb.raw("COUNT(CASE WHEN status = 'Delivered' THEN 1 END) as totalDelivered"),
          localDb.raw("COUNT(CASE WHEN status = 'Out for Delivery' THEN 1 END) as currentlyDelivering"),
          localDb.raw("AVG(CASE WHEN deliveredAt IS NOT NULL AND assignedAt IS NOT NULL THEN (julianday(deliveredAt) - julianday(assignedAt)) * 24 * 60 END) as avgDeliveryTimeMinutes")
        )
        .first();
      ...
      return {
        ...
        currentlyDelivering: stats.currentlyDelivering || 0,
        avgDeliveryTimeMinutes: Math.round(stats.avgDeliveryTimeMinutes || 0),
      };
    } catch (error) {
      throw error;
    }
  ...
  static async getAvailableDeliveryPersons(maxConcurrentDeliveries: number = 3): Promise<DeliveryPerson[]> {
    try {
      ...
      for (const person of activePersons) {
        ...
        if ((currentDeliveries?.count as number|| 0) < maxConcurrentDeliveries) {
